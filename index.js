		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD KAZINHO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Kanzinho? Este é um recurso especial para o Kanzinho ❌',
					ownerB: '[❗] Kanzinho? Este é um recurso especial para o Kanzinho ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de KZIN. Registre-se para fazer amizade com o bot Kzinho por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Kazinho|17\n\n──「 KAZINHO BOT 」──`,
			...
			const ownerNumber = ["5591984138022@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber = ['5591984138022@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5591984138022@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5591984138022@s.whatsapp.net"]//ubah nomor lo
			const premium = ["] 
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS KANZIMNHO BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Kanzinho ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT KANZINHO* 」\n*│+ wa.me/5591984138022 KANZINHO* 」*────*\n\n*_SE QUER SER ADMIN DO BOT KAZINHO_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5591984138022/*╰──────*「 * KAZINHO* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO KAZINHO BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do Criador:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* KANZINHO\n*WPP:* wa.me/+5591984138022\n*INSTA:* @Kanzinho_\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
